...
L["If checked, the sidebar will open automatically whenever you open up the auction house window."] = "若勾选此项，你每次打开拍卖行时都将自动打开侧边栏" -- Needs review
...
L["Module:"] = "模块:"
L["No modules are currently loaded.  Enable or download some for full functionality!"] = "目前未加载任何模块.启用或下载一些模块以使插件完全运作!" -- Needs review
L["Project Organizer / Resident Master Goblin:"] = "项目组织者/常驻大地精" -- Needs review
L["Provides the main central frame as well as APIs for all TSM modules."] = "为TSM所有模块提供主体框架及支持接口" -- Needs review
L["Slash Commands:"] = "指令:" -- Needs review
L["Special thanks to our alpha testers:"] = "特别感谢我们的测试人员:"
L["Status"] = "状态"
L["The only required module of TradeSkillMaster is the main one (TradeSkillMaster). All others may be disabled if you are not using them."] = "TradeSkillMaster唯一的一个必須载入的模块只有TradeSkillMaster,所有其他模块在你不需要的情況下都可以关闭." -- Needs review
L["There is a checkbox for hiding the minimap icon in the status page of the main TSM window."] = "在TSM主窗口的概述页面有一个复选框可以隐藏TSM的小地图图标." -- Needs review
...
L["TradeSkillMaster Team:"] = "TradeSkillMaster 开发组:" -- Needs review
L["Translators:"] = "翻译:" -- Needs review
L["Version:"] = "版本:" -- Needs review
L["Visit http://wow.curse.com/downloads/wow-addons/details/tradeskill-master.aspx for information about the different TradeSkillMaster modules as well as download links."] = "访问 http://wow.curse.com/downloads/wow-addons/details/tradeskill-master.aspx 以获取不同的TradeSkillMaster模块信息及其下载链接" -- Needs review
L["Want more tips? Click on the \"New Tip\" button at the bottom of the status page."] = "想要更多的小提示? 点击概述页面的\"新提示\"按钮即可。" -- Needs review
L["When using shopping to buy herbs for inks, it will automatically check if it's cheaper to buy herbs for blackfallow ink and trade down (this can be turned off)."] = "当使用Shopping来购买草药用于研磨制造墨水时, 插件会自动扫描是否购买制造秋闲墨水的原材料更便宜.(该功能可关闭)" -- Needs review
L["You can use the icons on the right side of this frame to quickly access auction house related functions for TradeSkillMaster modules."] = "你可以借助本窗口右边的图标来快速使用TradeSkillMaster模块中的拍卖行相关功能" -- Needs review
