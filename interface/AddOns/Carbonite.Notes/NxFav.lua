		...
		notesoptions = {
			...
			args = {	
				notemap = {
					...
					type = "toggle",
					width = "full",
					name = "Show Notes On Map",
					desc = "Shows your notes on the carbonite map",
					get = function()
...
function CarboniteNotes:OnInitialize()
	...
	Nx.Fav:Init()
	CarboniteNotes:RegisterComm("carbmodule",Nx.Fav.OnChat_msg_addon)		
	local function func()
		Nx.Fav:ToggleShow()
	end
	...
	Nx.NXMiniMapBut.Menu:AddItem(0, "Show Notes", func, Nx.NXMiniMapBut)		
	tinsert (Nx.ModuleUpdateIcon,"Fav")	
	tinsert (Nx.BarData,{"MapFav", "-Notes-", Nx.Fav.OnButToggleFav, false })	
	Nx.Map.Maps[1]:CreateToolBar()
	Nx.Map.Maps[1].Menu:AddItem (0, "Add Note", Nx.Fav.Menu_OnAddNote, Nx.Map.Maps[1])	
	Nx.Map.Maps[1].GIconMenu:AddItem (0, "Add Note", Nx.Fav.Menu_OnAddNote, Nx.Map.Maps[1])
	Nx:AddToConfig("Notes Module",notesConfig(),"Notes Module")
end

function Nx.Fav:OnChat_msg_addon(msg, dist, target)	
	local ssplit = { strsplit("|",msg) }
	if ssplit[1] == "CMD" then
		if ssplit[2] == "note" then
			Nx.Fav:SetNoteAtStr (ssplit[3])
		end	
	end
end
...
function Nx.Fav:Create()
	...
	local but = Nx.Button:Create (win.Frm, "Txt64B", "Record", nil, bw + 1, -bh, "TOPLEFT", 44, 20, self.But_OnRecord, self)
	self.RecBut = but

	local but = Nx.Button:Create (win.Frm, "Txt64", "Up", nil, bw + 48, -bh, "TOPLEFT", 40, 20, self.But_OnUp, self)
	local but = Nx.Button:Create (but.Frm, "Txt64", "Down", nil, 42, 0, "TOPLEFT", 40, 20, self.But_OnDown, self)
	Nx.Button:Create (but.Frm, "Txt64", "Delete Item", nil, 54, 0, "TOPLEFT", 72, 20, self.But_OnItemDel, self)
	...
	list:ColumnAdd ("", 1, 17)
	list:ColumnAdd ("Type", 2, 90)
	list:ColumnAdd ("Value", 3, 250)
	list:ColumnAdd ("Location", 4, 900)
...
function Nx.Fav:CreateMenu()
	...
	menu:AddItem (0, "Add Folder", self.Menu_OnAddFolder, self)
	menu:AddItem (0, "Add Favorite", self.Menu_OnAddFavorite, self)
	menu:AddItem (0, "")
	menu:AddItem (0, "Rename", self.Menu_OnRename, self)
	menu:AddItem (0, "Cut", self.Menu_OnCut, self)
	menu:AddItem (0, "Copy", self.Menu_OnCopy, self)
	menu:AddItem (0, "Paste", self.Menu_OnPaste, self)
	...
	menu:AddItem (0, "Add Comment", self.IMenu_OnAddComment, self)
	menu:AddItem (0, "")
	menu:AddItem (0, "Rename", self.IMenu_OnRename, self)
	menu:AddItem (0, "Cut", self.IMenu_OnCut, self)
	menu:AddItem (0, "Copy", self.IMenu_OnCopy, self)
	menu:AddItem (0, "Paste", self.IMenu_OnPaste, self)

	menu:AddItem (0, "")
	menu:AddItem (0, "Set Icon", self.IMenu_OnSetIcon, self)
end
...
function Nx.Fav:Update()
	...
	self.Win:SetTitle (format ("Favorites: %s", self.FavCnt))
end
...
function Nx.Fav:CreateItem (typ, flags, name, p1, p2, p3, p4)
	...
	elseif typ == "N" then	-- Note
		...
		s = self:MakeXY (p3, p4)
		return format ("N~%c~%s~%c%02x%s", flags, name, p1 + 35, id, s)

	elseif typ == "T" or typ == "t" then	-- Target

		local id = Nx.MapIdToNxzone[p1]
		s = self:MakeXY (p2, p3)
		return format ("%s~%c~%s~%02x%s", typ, flags, name, id, s)
...
function Nx.Fav:ParseItemNote (data)
	...
	local zone = tonumber (strsub (data, 2, 3), 16)
		...
		if type (zData) == "table" then
			...
			for id, data in pairs (zData) do
				...
				if not mId then
					...
					Nx.prt ("Import %s %s %s %s %s", name, zName, mId, x, y)
				end
			end
		end
	end

	Nx.prt ("Imported %s notes", importCnt)

	self:Update()
end

function Nx.Fav:OnButToggleFav (but)
	Nx.Fav:ToggleShow()
end
